...
**In:** the ranked full list. **Out:** the page slice (`per_page`, default 20 — [search-pipeline.ts](../../apps/backend/src/services/search-pipeline.ts), `params.perPage ?? 20`) with exact count and page arithmetic, plus — when the `clusterPool` lever is on — the **clustering pool**: the top candidates down to a relevance boundary, independent of the page. Pool modes ([ranking-config.ts](../../packages/domain/src/ranking-config.ts)): `page` (legacy default — the pool is the page window), `fixed:<n>` (sweep comparison arm), and `score-gap` (the measured candidate: extend to the largest adjacent score drop found between `minSize` and `size` candidates — a relevance-bounded boundary rather than a global cutoff, per the [pool-sizing memo](../planning/ranking-research/candidate-pool-sizing.md)).

Honest assessment: the 20 is an inherited web-pagination default with no recorded performance or quality rationale, and on the check path it is doing a second job it was never designed for — it is also the **clustering pool**. The engine's other surfaces already size that pool independently: the Recipe Map passes *"perPage: 10000, // no pagination limit — clustering IS the pagination"* ([traces.ts](../../apps/backend/src/routes/traces.ts)), briefing exemplars use 10,000, and the eval runner uses 100 — the check path is the only consumer clustering a page-sized pool. Performance does not force the small window at current scale: the map k-means over the whole corpus after MRL-truncating vectors to 768 dims, *"4× less transfer/parse/k-means work than the full 3,072 dims"* ([search-pipeline.ts, `MAP_VECTOR_DIMS`](../../apps/backend/src/services/search-pipeline.ts)).

Why this matters: clustering's purpose is diversity, and a fixed similarity-ranked window caps the diversity available to it — the richer the corpus, the more homogeneous its top 20. The operator's framing of the open question, verbatim: *"It feels like the point of clustering is diversity, and that the default 20 limits this. So this might be our actual 'echo chamber' issue."* (Andy, design review 2026-07-16, recipe `bb952d78`). That is hypothesis [P6](#presentation--budget), and its lever now exists: `clusterPool` ([ranking-config.ts](../../packages/domain/src/ranking-config.ts)) can decouple the clustering pool from the page — `fixed:100` draws the summary from the top 100 candidates (the retrieve-~100-then-refine convergence point of production practice, per the [pool-sizing memo](../planning/ranking-research/candidate-pool-sizing.md)) at 768-dim pool vectors, while flat pagination stays untouched. It ships in `page` mode (legacy, byte-stable) pending the sweep. A naming honesty note travels with it: when results are clustered, the *cluster count* is what the caller actually receives, so "pagination" describes the flat/`expand` mode better than the default clustered mode.
...
**In:** everything above, plus the caller's known-set: this session's prior deposits (via the opaque `session_id` token — self-healing, echoed back as `data.sessionId`) ∪ client-declared `known_recipes` ids. **Out:** the response — HTML, JSON (`format=json`), or MCP text/structured. Known results render as **id-only stubs** at their true rank (no gist — a truncated claim can be read as the claim; fetch the full recipe by id via `get_recipes`), and a known cluster exemplar is replaced for display by its cluster's next-nearest non-known member with the known id carried as a stub — *"return the ID instead, and then crucially, return the full text of the next recipe in line"* (Andy, 2026-07-16, recipe `4d25aec9`). The display budget is spent on novel content; ranking and membership never change (seam 2). The `ranking` block (`{version, clusterPool}`) stamps every JSON/structured response with which algorithm produced it; the premium `synthesize` distillation also lives here, off the ranking path.
