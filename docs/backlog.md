...
### `[DECISION NEEDED]` Trim the hosted (gemini) Docker image after local-embeddings

The local-embeddings implementation ships **one provider-agnostic image**: `@huggingface/transformers` + `onnxruntime-node` are installed and the default local model is baked unconditionally, so the hosted **gemini** image grew ~100–300 MB (deps) + ~23 MB (baked weights) for code it never executes (the dynamic-import gate means no *runtime* cost — image size only). This diverges from the planning doc's stated intent that gemini/stub images stay unaffected; the shared-image approach was chosen for operational simplicity. Decide whether the hosted image size warrants (a) a conditional model bake via a build ARG, (b) pruning the optional dep for non-local builds, or (c) provider-specific image variants — vs. accepting the larger single image. If the hosted deployment's image size / cold-start matters, this is the lever. Also: a heavy env-gated CI job that runs a real `local` bge-small semantic-similarity smoke test on the Debian image (HF_HOME cached) was scoped but not built — schedule if wanted.
