...
**Model/provider**: Gemini-class in prod — the operator's `GEMINI_API_KEY` already powering embeddings; same key, new env `SYNTHESIS_MODEL` (default `gemini-2.5-flash`). Keep the call behind a small provider seam (one function) so models swap by env. **Explicit non-goals now**: no per-user quota tracking, no rate budget, no billing — the user base is the operator and trusted individuals (that's *why* premium is manual). A LiteLLM-router deployment for per-user quota is a separate backlog item (added under Infrastructure in `docs/backlog.md`), so do not hand-roll quota here.
