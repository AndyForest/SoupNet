...
- `/health`, `/health/ready`, `/health/version`, `/health/integrity` — liveness + DB readiness (both public) plus API-key-authed stack introspection and index-integrity check. Point a load balancer / health-check probe at `/health/ready` if the deployment uses a rotating database password (the `SELECT 1` lets the probe detect auth drift after rotation and trigger task replacement). `/health/version` returns git commit, ranking-algorithm version, migration head, embeddings provider/model, and the presenting key's expiry — the eval-reset contract's stack-introspection surface so a benchmark runner can check stack staleness + its own key runway in one authenticated GET. `/health/integrity` (eval-reset contract item (b1)) reports orphaned embedding sources/chunks/vectors per readable book (scoped to the key's `read_group_ids`) so an eval runner can prove index-consistency cheaply before a scored run.
...
The only anti-pattern is checking a recipe you don't genuinely believe — that degrades future checks for everyone. If you're just looking for something, use the read-only search rather than fabricating a recipe: the `search_recipes` MCP tool or the web endpoint's `filter` (alias `f`) param. Both take structured queries — bare text is semantic, `"quoted terms"` match lexically (filenames for PR review), `author:`/`after:`/`before:` filter structurally. Search is the tool for finding *collaborators'* judgment calls (its MCP results exclude your own recipes by default); with neither a genuine hypothesis nor an other-author need, ask the user or read the project's own sources.
