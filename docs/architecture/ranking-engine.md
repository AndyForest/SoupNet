...
| P6 | The fixed candidate window, not authorship, may be the deeper echo-chamber mechanism: *"the point of clustering is diversity, and that the default 20 limits this. So this might be our actual 'echo chamber' issue."* — a clustering pool sized independently of pagination should restore diversity as the corpus grows (the check path is the only surface clustering a page-sized pool; see [stage 3](#stage-3--the-candidate-window)) | Andy, design review 2026-07-16 (recipe `bb952d78`); [brief §3d](../planning/check-recipe-ranking-system.md) *"evaluate a pool-composition lever alongside"* | A pool-size `RankingConfig` lever at stage 3, decoupled from `per_page` | notes only |
