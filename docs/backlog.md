...
### `[IMPL]` Cold-start v2 — thin briefing, intents, book selection (operator-approved plan 2026-08-23; Phases A+B+C implemented same day — stacked branches in review; Phase D remaining)

The cold-start program for fresh agent sessions on a rich corpus (design rulings in soupnet-oss recipes `363e3e0c`, `401998c5`, `5c55327d`, `ef844c32`). Phase A (gap fixes + surface plumbing) shipped 2026-08-23: short-id prefixes on `get_recipes`/`GET /recipes` (+`ambiguous_prefix` marker), briefing exemplar fallback for single-trace books, honest Logged/Decided exemplar labels, zero-result own-exclusion disclosure (`buildOwnExcludedNote`, shared notice builder), `briefing.issued` surface stamps on all entry points, description cap unified at 2000, search-feedback confirmation names its target. Remaining phases, each its own draft PR:

- **Phase 0 (live experiment, zero code):** nightly scribe maintains per-book digests inside `groups.description` via `update_recipe_book_description` — brief at [docs/briefings/nightly-scribe-brief.md](briefings/nightly-scribe-brief.md). Guidelines-not-prescriptions (descriptions are agent-maintained, recipe `26751d76`); what it teaches decides whether a dedicated digest column ever exists.
- ~~**Phase B — thin MCP briefing + book index.**~~ Implemented 2026-08-23 (`feat/cold-start-briefing-index`, stacked on Phase A): surface-profiled `BRIEFING.build` (mcp drops setup cluster / link formatting / pasted-JSON / web-divergent paragraph; full = web/paste, byte-stable), per-book Index lines from `book-stats.service` (three GROUP BYs, MCP surfaces only), MCP exemplar default 0 with verbosity opt-in, specs in `briefing-surfaces.feature` + spec-decision-log entry (log renamed from declared-intent-log.md).
- ~~**Phase C — intents.**~~ Implemented 2026-08-23 (`feat/cold-start-intents`, stacked on B): migration 0036 (`intents`, `intent_shown`, `check_feedback.intent_id`), `intent.service` (always-new registration, own budget, join-only feedback per `abddb65d`), self-healing `intent` param across briefing/check/search on all agent surfaces, delivered-set stub walk-down via knownIds (ranking-regression invariants unchanged), audit `metadata.intentId`, trace-detail API exposes the feedback join. The search-path session known-set 7-day window asymmetry fixed in its own commit. Live-verified per testing-plan 4b. Remaining from this phase: retention decision for stored stories (audit-retention pass); search-grammar friction note — natural-prose parentheses in semantic queries hit loud group-grammar errors (found authoring the Layer 3 tests; consider quoted-terms-only paren grammar or a workaround-naming error message).
- **Phase D — purpose-driven book selection arm** (off-by-default; governing handoff constraints: default path unchanged, rank-relative top-N only, `recipe_ids` full-scope, explicit `recipe_book` wins, degrade to full scope). Two-stage via the search pipeline as a normal client — not the fragile goal-mmr seam. Eval: regression ceiling only; real multi-book golden set before any flip talk.
- **Session-system deprecation (decided, not yet executed):** intent supersedes `session_id` (same stub machinery + story payload + compaction survival + feedback join); `session_id` stays honored, deprecation-marked, guidance steers to intent. Open convergence question: auto-mint an anonymous intent when neither param is given?
- **`[DESIGN]` Custom briefings modal (web):** dashboard briefing buttons open a modal — target-AI-agent dropdown (per the public connect page's client list) + intent text field → briefing with client-appropriate setup only, intent-biased exemplars, registered intent id embedded.
- Also parked from Phase A: check-page hand-rolled books panel should unify with `renderRecipeBooks` when Phase B's index lands; flat-mode search results flag knowns but don't backfill (intent stubbing inherits this; eval-gated follow-up).
...
### `[IMPL]` Replace `clusters`/`max_chars` with one adaptive `verbosity` lever on check + briefing surfaces

Triggered by the evals-side finding that `max_chars` is a silent no-op on MCP ([finding-max-chars-mcp-contract.md](planning/finding-max-chars-mcp-contract.md)). Design **operator-ratified 2026-07-26**: [planning/response-verbosity-lever.md](planning/response-verbosity-lever.md) — `verbosity: low|medium|high`, omitted = automatic (adaptive-k via largest-gap/knee on the pool similarity curve per Weaviate autocut + Adaptive-k EMNLP 2025); no hard character caps (anti-pattern; LLM APIs deprecated numeric budgets for effort enums + adaptive defaults); legacy params deprecated-marked but honored with the one-line MCP precedence fix; monotonicity regression tests per surface. Scope includes `get_briefing` (default unchanged; verbosity maps to exemplar count) plus plumbing the `goal-mmr` briefing selection-mode arm off-by-default for the future adaptive-briefing experiment (the existing `purpose` param is the goal-summary input — don't duplicate it), and public algorithm-docs updates with the research references. Adaptive MMR λ (variance-keyed per Wang & Zhu 2009) and the adaptive-k automatic realization both ship behind ranking-config levers, flipped only by measurement. Implementation order in doc §8.
...
Diagnosed 2026-07-21 during the ephemeral-workspaces gate: three rotating single-test 15s timeouts across full-suite runs, reproducible, with a live `pg_stat_activity` sampler proving ZERO database queries exceeded 1s — the hangs were event-loop starvation, not DB. `bcryptjs` (pure JS) runs every password hash/compare on the backend's single thread; the gate's parallel test files register dozens of users concurrently and the resulting hash convoy starves whichever bcrypt-path request queues last. Production implication (security-adjacent — follow docs/workflows/security.md): an unauthenticated `POST /auth/register` burst can degrade the whole API the same way. Candidate fixes: native `bcrypt` (libuv threadpool — weigh the node-gyp/self-hoster build cost against the deployment-agnostic principle), or a small in-process semaphore capping concurrent hash operations (no dependency change, bounds the convoy). Interim: the three known-marginal tests carry 45s timeouts with the diagnosis cited in check-hardening.test.ts.

### `[IMPL]` User-delete cascade (cheap teardown for import-based testing)

Operator request 2026-07-19. The "create a user, import an export, test, delete the user" pattern is fully supported on the way in (registerUser + POST /import) but has no teardown: deleting a user does not cascade to their orgs/books/traces/evidence/links/embedding rows, so test corpora imported into a shared database have no clean exit. A user-delete cascade (or an equivalent "delete book with all contents" primitive — the evals side's hygiene wishlist independently asks for a "reset book" command) would make in-place isolated testing viable without scratch databases. Design notes: must respect the audit-log append-only trail (record the deletion, don't erase history), and shared-book membership needs a rule (delete only books the user solely owns; leave co-owned books with a departed-member marker). Security-relevant (data deletion surface) — follow docs/workflows/security.md and add authorization tests.

### `[DECISION NEEDED]` Should user data export/import speak the canonical Recipe schema?

Post-#40 schema-pinning sweep (2026-07-18): `GET /auth/me/export` and the corpus import (`import-validate.ts`, "the inverse of GET /auth/me/export") round-trip with each other using the raw DB graph (`claimText`, `groupId`, flat `traceEvidence`/`traceReferences`/`evidenceReferences` link tables, `schemaVersion: 1`) — neither speaks the canonical Recipe object of `packages/contracts/src/recipe.ts`. This is a deliberate lossless-backup design (the link tables carry `stance` and N:N structure the nested Recipe shape flattens), so it was classified as a product decision, not drift. Options: (a) leave as-is — backup format is internal, canonical Recipe consumers use `GET /recipes`; (b) additionally accept canonical Recipe arrays on import (agent-produced corpora become importable); (c) add a canonical-Recipe export flavor (`?format=recipes`) for portability into other tools. If (b) or (c), bump `schemaVersion` semantics deliberately and keep the raw graph as the fidelity path.
...
Update (2026-09-19): the guard now protects only the shared organization itself. Recipe books in a *personal* organization never block deletion — `deleteUserCascade` hands a book that other people belong to on to a remaining member (another owner, else the longest-standing admin, else the longest-standing member), re-homes it into that person's personal organization, and deletes only what the departing user authored. Two follow-ups this leaves open: (1) an in-app way for an owner to pick their successor *before* leaving, rather than accepting the default order; (2) how a non-personal organization changes hands belongs to the organization-accounts program, and the guard should be revisited there (org-level membership is not visible to its `group_members`-based check). Also open: the new owner is not notified that a book passed to them — the only record is the `recipe_book.ownership_transferred` audit row.
...
`decided_at` shipped 2026-06-10 (column, `/check` + MCP param, coalesced judgment date in search results, trace-detail "Decided … · logged …" display). ~~Briefing exemplars~~ done 2026-08-23 (cold-start v2 Phase A) — and the prior state was worse than "shows only created_at": exemplars printed the COALESCED judgment date under the label "Logged:", erasing the backfill distinction; they now render Logged (append time) + Decided (only when different), matching the Requested-recipes renderer. Remaining surfaces that still show only `created_at`: the traces list pages (`/app/traces`, recipe-book traces) and the Recipe Map tooltips. Low urgency — agents already see the coalesced date in check results, which is the surface that matters for alignment.
...
### `[DESIGN]` Per-slot embeddings (aspect channels) + reversal-aware presentation — slot-lab measured separation 2026-07-21

Data-science phase complete (operator-scoped deliverable 2026-07-21: "just data-science rather than full new product features/surfaces"); everything lives in gitignored `eval/golden/slot-lab/` (runbook.md declares the question; report.md + metrics.json carry the numbers; scripts regenerate against a throwaway rankeval postgres — no product wiring, nothing registered in the strategy registry, production search untouched). Measured on the operator's real 1,815-trace corpus export under gemini: anchor-split slot texts (role / goal / decision / reason — grammar enforced by format-adherence, so splitting is delimiter work, coverage 97.9/77.4/95.8/88.1%) embed into near-independent channels — a recipe's role channel vs its own decision channel sits at the random-cross-recipe baseline (0.640), the role channel ranks the corpus almost orthogonally to full-text (Spearman 0.20, median NN-overlap@10 = 0), and cross-channel similarity correlations run 0.06–0.41. Near-twin mining (full-sim ≥ 0.85) found 695 non-identical pairs; the decision channel separates *reworded* decisions (mean 0.814) from same-worded ones (0.973) but cannot label *stance* — reversal detection stays wording+dates+human arbitration per ADR-0015. **Ruling (2026-07-21): minimal approach shipped** — authoring guidance instead of machinery: the recipe guide's §Authoring for retrieval now carries the changed-mind paragraph with measured numbers, sourced from [the stance-separation memo](planning/ranking-research/stance-separation-probe.md) (which also holds the operator-named labeled pairs: real justification-rich reversals measure 0.70–0.74 full-text — ordinary neighbors, not near-twins; a bare flip at 0.86 is indistinguishable from a rephrase at 0.88). One labeled pair is still missing: the EC2 smallest-viable-instance → memory-upgrade arc's older recipe matched no phrasing sweep (`2eef8dc6` is the newer side); ask the operator for clues if the labeled set matters later. Follow-ups, each gated on an operator ruling: (a) `exp_slot_*` strategy-registry registration (the graduation step — note the worker backfills corpus-wide with no scoping, and the admin embeddings toggle does NOT gate the worker's batchEmbed path); (b) aspect-based display selection (slots-as-aspects, xQuAD/PM-2 lineage) as a `displaySelection` arm through the sweep → report → ruling path; (c) reversal-aware presentation (near-twin τ + `decided_at` tie-break + annotated stubs — S6's shape); (d) the directed feedback edge (`superseded_by`/`contradicted_by`/`confirmed_by` with provenance) — `check_feedback.related_trace_ids` is a bare UUID array today, and feedback anchors on the *check*, not the returned recipe a discount signal would need to point at. Hypothesis-register rows (ranking-engine.md) land with whichever follow-up branch first wires a lever. **Synthetic stance probe (2026-07-21, `slot-lab/reversal-report.md`, 24 real recipes × paraphrase/minimal-negation/nuanced-reversal):** gemini separates nuanced reversals from paraphrase controls 24/24 paired on full-text (0.858 vs 0.960 mean) — better than the NevIR negation literature predicts — but *minimal* negations stay near-blind (0.903 full, decision-channel 19/24 weakly separated), so the separation comes from the new reasoning/wording, not stance comprehension; and 14/24 reversals still sit at/above the 0.85 near-twin gate, so MMR's redundancy penalty suppresses a fresh reversal shown beside its predecessor — the (c) reversal-aware presentation follow-up stays necessary. The validated detector fingerprint: role+goal channels high ∧ full-text 0.75–0.90 ∧ decision/reason channels low. The earlier "no reversals in the top-40 eyeball" was a selection artifact: that list sorted by textual twinness, which reversals' ~0.10 separation pushes below the horizon.
...
Second race, observed 2026-08-19 (chore/deps-security gate run; failing assert is `workspaces.test.ts:308`, `expect(after).not.toContain(slug)`): expire-now stamps `expires_at` from the Node host clock (`new Date()`, `routes/workspaces.ts`), but the briefing-scope filter compares against Postgres `NOW()` (`services/briefing.ts`) — the DB container's clock. Any skew between host and Docker/WSL2 VM clocks larger than the ~10ms between the two HTTP calls leaves the book "live" in DB terms while the tombstone response (host-clock comparison) already says `tombstoned: true`. **Fixed 2026-08-19** (rode on `chore/deps-security-2026-08-19` after blocking 2 of 3 gate runs; host-vs-VM skew measured at ~300-600ms that day): single-clock discipline — "now" is stamped as `NOW()` inside the UPDATE and `tombstoned` computed in the same RETURNING clause, so expiry, verdict, and every filter read the DB clock; host-supplied ISO timestamps for future expiries are unaffected. The reaper race at `:318` (first paragraph) remains open.

### `[IMPL]` CI flake: sync-embed-path.test.ts races the worker strategy sweep

Observed 2026-08-23 on PR #76's GitHub Actions run (passed on three same-tree local gates): "generates SEMANTIC-only sync-complete vectors" asserts a fresh trace carries only the two sync-written strategies, but the in-process strategy sweep (backfills `exp_*` within ~1 minute) fired before the assert on the slower CI runner — received `exp_trace_instructed` + 2 more. Same class as the workspaces reaper race below: asserting an intermediate state without pausing the consumer that erases it. Fix options: pause/lengthen the sweep interval for this test, or assert the sync-written strategies are a SUBSET present immediately (the actual invariant) rather than the exact set.

### `[IMPL]` test:ci flake: vitest worker fork "exited unexpectedly" after all tests pass

Observed 2026-07-21 (feat/feedback-web-surface, first gate run): 924/924 tests passed but vitest reported "Unhandled Error: [vitest-pool]: Worker forks emitted error / Worker exited unexpectedly" at teardown, which fails the gate; the immediate re-run passed clean with an identical working tree. No test failed in either run, so this is a teardown-phase crash (open handle, OOM, or child-process race) in one of the integration files, not a logic failure. If it recurs, re-run with `--reporter=verbose` to identify which worker/file dies post-pass before treating any green-tests-red-gate run as real. **Recurred 2026-09-19** on `feat/authz-seam` (parallel gate, `TESTCI_PGPORT=5544`): of four runs of effectively the same tree, two passed (1101 tests) and two failed without any test assertion failing. One was this worker-exit shape, but mid-run rather than at teardown (1095 passed, six tests never reported). The other was a different shape: the CI backend passed its health check and then refused connections on its port for the rest of the run (38 files failed with `ECONNREFUSED`), with no crash output captured. Free memory was ample (31 of 63 GB), so not memory pressure. The verbose reporter printed no per-file lines when output was redirected to a file, so the dying worker is still unidentified; next attempt should use `--reporter=junit` or `--reporter=json` with an output file, and the gate script should capture the backend's exit code and stderr so a silent backend death is visible.
...
- **`/check` claims "No API key provided" while a valid `Authorization: Bearer` header is present** — it only reads `?key=`. Pre-existing, not from this batch. Either accept Bearer or say "pass it as ?key= on this endpoint." Related: the `/docs` `?key=` prefill audit item. (2026-07-21: the new `GET /feedback` accepts both `?key=` and Bearer — the convergence pattern to extend here.)
- **`get_recipes` rejects 8-char short ids while the feedback surface accepts them** (found 2026-08-20 by a pr-brevity benchmark agent following the ranking changelog's "Lineage recipes: `ee4479c4`, `257950f3`" pointers): in-tree docs and PR bodies cite recipes by short id, feedback rows resolve 8+ char prefixes server-side (within-scope, uniform markers — recipe `507d3c9c`), but the lookup tool demands full UUIDs, so the natural pointer-following flow dead-ends into a search round-trip. Extend the same within-read-scope prefix resolution to `get_recipes` / `GET /recipes`.
...
The 2026-07-12 ship resolves short ids on `log_feedback` / `feedback` param / `POST /feedback` `trace_id` only. `related_trace_ids` stays full-UUID deliberately (capture-only lineage; resolving turns capture into validation). ~~`get_recipes` / `GET /recipes`~~ done 2026-08-23 (cold-start v2 Phase A — demand appeared 2026-08-20 from two independent benchmark agents that held 8-char prefixes confidently but garbled full-UUID transcription; same within-read-scope resolution as feedback, plus an `ambiguous_prefix` status naming in-scope candidates). Remaining candidates if demand appears: `known_recipes`, briefing `recipe_ids`.
...
- ~~**nodemailer high — `raw`-option file-read/SSRF (GHSA-p6gq-j5cr-w38f).**~~ Resolved — nodemailer is at 9.0.3 (verified 2026-08-19), the advisory's fix version.
...
**2026-07-22 remediation** closed 7 of 8 open Dependabot alerts via a full lockfile refresh (fresh resolve under the `min-release-age=7` quarantine): fast-uri host-confusion GHSA-4c8g (3.1.3), sharp/libvips GHSA-f88m (0.35.3 via a new root override — @huggingface/transformers still pins ^0.34.5 upstream), linkify-it DoS GHSA-v245 (5.0.2), the js-yaml merge-key DoS GHSA-52cp copy orval pinned at 4.2.0 (the existing `$js-yaml` override finally applied on fresh resolve), and the @hono/node-server serve-static path traversal GHSA-frvp across all three manifests (1.x→2.0.9 direct bump in backend+mcp-server plus a root override for @modelcontextprotocol/sdk's ^1.19.9 pin — SDK has no 2.x-compatible release). The orval-8 output regeneration flagged in the note above shipped with it. ~~**Two quarantine-blocked residuals, one follow-up bump closes both:** @hono/node-server 2.0.10 (WebSocket-handshake memory-leak DoS GHSA-9mqv, moderate; published 2026-07-15, ages out ~2026-07-23) and fast-uri 3.1.4 (literal-backslash host confusion GHSA-v2hh, high; published 2026-07-19, ages out 2026-07-26). Run `npm update fast-uri @hono/node-server` on/after 2026-07-26 and commit the lockfile.~~ Done in the 2026-08-19 batch below.

**2026-08-19 remediation** closed all 15 open Dependabot alerts' runtime-scoped entries via targeted `npm update` under the quarantine (branch `chore/deps-security-2026-08-19`): hono 4.13.1 (memo SSR cross-user disclosure, CORS ReDoS, language-middleware DoS, proxy Connection-header — all four alerts), @hono/node-server 2.1.0 (WebSocket-handshake memory-leak DoS GHSA-9mqv), fast-uri 3.1.5 (both backslash host-confusion highs), ip-address 10.5.0 (all three SSRF/trust-boundary-bypass alerts), tar 7.5.22 (mapHas recursion DoS), @modelcontextprotocol/sdk 1.30.0 (rides along, PR #64 pairing). `npm audit fix` plus a root js-yaml 4.3.0→4.3.1 pin bump also cleared brace-expansion (high), nanoid (high), js-yaml omap DoS (high), postcss, and the dev-scoped js-yaml Dependabot alerts. The corresponding Dependabot PRs (#64, #65, #66, #69, #70) auto-close when the versions land on main. New accepted dev-only residual, with upgrade path per the pattern above:

- **extract-zip high — unvalidated symlink path traversal (GHSA-jmr9-qjv8-65gv).** Dev-only (puppeteer → @puppeteer/browsers). Only fix is a puppeteer 24→25 semver-major. Upgrade path: bump puppeteer 25.x in its own change and re-run whatever exercises it (screenshot/eval tooling); not reachable from any runtime surface.
