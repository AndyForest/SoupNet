...
### `[IMPL]` Cold-start v2 — thin briefing, intents, book selection (operator-approved plan 2026-08-23; Phase A shipped same day)
...
- **Phase B — thin MCP briefing + book index.** Specs first (new `.feature` files — behavioral-specs phase 1's forcing function). MCP surface drops setup cluster + clustered exemplars at default (`verbosity=high` restores); per-book index line from deterministic SQL (extend the map-cache fingerprint GROUP BY, judgment-date recency, feedback/reaction rollups). Web/paste briefing byte-unchanged (per-surface snapshots are the guard). How-to mechanics move to tool descriptions.
- **Phase C — intents.** Optional self-healing `intent` param on briefing/check/search (+ join-only on feedback, capture-only leniency per `abddb65d`): text always registers a NEW intent (never dedup — identical phrasing across sessions must not merge, operator ruling) returning `int_`-prefixed id; `intents` + `intent_shown` tables (session_shown posture, rendering-only); delivered recipes stub-render via the existing knownIds walk-down (ranking purity holds, `9067ca1b`/`4d25aec9`); `check_feedback.intent_id` + audit `metadata.intentId` close the declared-intent→fulfillment loop. Fix the session knownIds window asymmetry (check 7d, search none) in its own harness-gated commit. Retention decision for stored stories folds into the audit-retention pass.
