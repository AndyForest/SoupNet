...
3. **A real deficit on cross-event integration** (0.766 vs 0.827, n=423): tasks whose answers integrate many events favor having everything in context over top-k retrieval. A retrieval-breadth ablation (2.5× exemplars, 3× context budget — existing public API parameters, no product changes) **did not close the gap and mildly degraded other slices** (type-3 0.733, type-2 0.738 at 2,208 mean ctx tokens): more marginal recipes dilute rather than help, so the gap looks structural (integration wants synthesis, not breadth). The ablation also produced a free calibration: its no-memory rows receive byte-identical inputs across runs and still moved 0.411 → 0.326, bounding answering-model run-to-run noise at roughly ±0.05–0.08 per slice — a bar any single-run delta here (including our own "parity" reading) must clear before being sold as signal. Repeat-run methodology is the follow-up, not more retrieval.
