...
**2026-08-19 snapshot (overdue):** the two quarantine-blocked bumps below were never run and GitHub now reports **17 open alerts** (6 high / 9 moderate / 2 low). New since July, runtime-relevant: a second `fast-uri` high (GHSA-7p8r), `ip-address` high + 2 moderates, `tar` moderate, and four advisories on `hono` itself (GHSA-f23p, GHSA-8j4g, GHSA-54fx moderate + GHSA-79qm low). Dev-only new: `js-yaml` ×2 high, `extract-zip` high, `postcss` moderate. Next session: run the scheduled `npm update fast-uri @hono/node-server`, bump hono/ip-address/tar within semver, re-assess the rest against the accepted-deferral list under the security workflow.
