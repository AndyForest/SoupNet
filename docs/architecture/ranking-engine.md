...
flowchart TD
    ...
    D --> E["3 · Candidate window<br/>out: the top slice by rank<br/>(default 20 — see P6)"]
...
On "how loose is the matching": there is no looseness knob and no score threshold — cosine similarity orders *all* candidates (principle 2). What varies is how much of that ordered list survives into the displayed summary, which is stages 3 and 5 — and the size of that survival window is itself an open lever ([P6](#presentation--budget)).
...
### Stage 3 — the candidate window

**In:** the reordered full list. **Out:** the top slice by rank (`per_page`, default 20 — [search-pipeline.ts](../../apps/backend/src/services/search-pipeline.ts), `params.perPage ?? 20`) plus the exact total count and page arithmetic. This is the first place the list gets shorter — *after* scoring, so a demoted recipe can move to a later window but never off the result set — and everything downstream (clustering, evidence, rendering) summarizes **this window**.
...
| S3 | Human *still true* reactions and cross-agent fulfilled feedback protect genuinely reinforced recipes without shielding echoes (principle 3's provenance axis) | [Feedback-calibration audit 2026-07-09](https://www.soup.net/traces/ff54eafd-b22f-476e-93a1-cf3fe0da0fc8) | Two exemption flags on the stage-2 occupant (shipped OFF) + lazy signal counts | plumbed |
...
| P6 | The fixed candidate window, not authorship, may be the deeper echo-chamber mechanism: *"the point of clustering is diversity, and that the default 20 limits this. So this might be our actual 'echo chamber' issue."* — a clustering pool sized independently of pagination should restore diversity as the corpus grows. Research converged on ~100 (*"a hundred documents on input is probably the minimum for any statistical significance"* — Carrot2, [pool-sizing memo](../planning/ranking-research/candidate-pool-sizing.md)) | Andy, design review 2026-07-16 (recipe `bb952d78`); [candidate-pool-sizing memo](../planning/ranking-research/candidate-pool-sizing.md) | `clusterPool` lever at stage 3 ([ranking-config.ts](../../packages/domain/src/ranking-config.ts)): `page` (legacy default) vs `fixed:100` @ 768-dim pool vectors; score-gap adaptive variants are the sweep's alternates | plumbed |
