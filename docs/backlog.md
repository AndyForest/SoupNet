...
### `[IMPL]` Corpus-version key for the Recipe Map cache (structural fix)

The map's layout cache derives a corpus-version key from `count(*)` and `max(created_at)` **aggregated across the union of the requested books** (`apps/backend/src/routes/traces.ts:104-108`). Aggregating over the union before hashing destroys per-book information: any operation that moves a trace *between* two books in the union leaves both statistics unchanged, so the cache serves a stale layout indefinitely. The move-recipe work ships the minimal correct fix — fingerprint per book (`GROUP BY group_id`), combine in stable order, plus `max(updated_at)` to catch in-place row edits that no insert-time statistic can see.
