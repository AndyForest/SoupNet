...
- **Human SPA** at `:5173` — public pages (clean unauth'd context), authenticated pages, admin pages (DEV user has the system role).
- **Agent-facing HTML** at `:3001` — the four `/docs/*` reference pages served by the backend for AI agents reading via `fetch`.
...
1. **Dev servers running** — frontend on `:5173` and backend on `:3001`. Easiest: `docker compose up -d` and `npm run dev:frontend`.
...
4. Opens a third unauth'd context against `:3001` and captures the agent-facing HTML pages (`/docs/recipe-check-guide`, `/docs/recipe-scenarios`, `/docs/mcp-setup`, `/docs/bootstrap`).
...
- **Backend `:3001/check`** — the agent-facing recipe-check form. Requires an API key in the URL, which would expose the key in the screenshot. Capture manually with a short-lived throwaway key if needed.
